...
bool Socket::bind(NetAddress address)
{
    ...
    if(ret == -1) std::cout<<"[bind] error"<<std::endl;
...
bool Socket::listen(int backlog)
{
    ...
    if(ret==-1) std::cout<<"listen error"<<std::endl;
    return (ret==-1)?false:true;
}
...
Socket Socket::accept(NetAddress* clientAddress)
{
    ...
    if(ret == -1)
    {
        std::cout<<"[Socket] accept return -1 "<<std::endl;
        return Socket();
    }
    return Socket(ret);
}
...
void Socket::setNonBlocking(bool tag)
{
    if(tag)
